...
- `WORKLAB_BUILTIN_TOOLS` (`src/core/builtin-tools.js`) must contain the exact `Agent` name as well as the Claude-compatible `Task` alias and `Skill`. Worklab always sends a named `allowedTools` array, so omitting a native entry silently removes that CLI capability. Keep the other supported native helpers too; `TodoWrite` alone is deliberately excluded because `core/run-todos.js` owns run todos.
