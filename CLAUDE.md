...
- The plan stage has no working route on an `allow_all_only` runtime, so a Codex/OpenCode agent cannot run it. Translating read-only planning to the provider's native `permissionMode: "plan"` is the obvious fix and does **not** work — do not re-add it without re-testing. Under Codex's read-only sandbox every MCP tool call is gated behind `mcpServer/elicitation/request`, and the runtime refuses all server-initiated requests and kills the turn; measured with one `journal_append`, `bypassPermissions` returns `{"ok":true}` while `plan` stops the turn. Since agents depend on the worklab MCP tools, refusing to start beats dying mid-run. Tracked upstream as [mono-agent#553](https://github.com/robertsreberski/mono-agent/issues/553); [#552](https://github.com/robertsreberski/mono-agent/issues/552) (plan mode pins `networkAccess: false`) also has to land before this is worth revisiting.
