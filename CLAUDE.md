...
- Agent-runtime 0.17 added optional same-model route attempts, request-scoped `toolEnvironment`, and generic Pi `Agent` subagents. Worklab does not opt into those surfaces: fallback chains retain one attempt per entry, the worker owns its environment, and existing native-subagent policy remains the only subagent path. `src/__tests__/core/ai-runtime-options.test.js` guards that boundary.
