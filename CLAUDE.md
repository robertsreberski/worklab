...
- **Subagents are not the team roster.** A team roster exists so a planner can create durable child tasks through `decision: "delegate"`. Native subagents are ephemeral in-run helpers and come from the *runtime's own on-disk profiles* (`~/.claude/agents`, `.claude/agents`). These were one mechanism until v49; conflating them meant the subagent tool was withheld from every task without a team, which is the default. Do not reintroduce a roster-derived subagent context.
- `WORKLAB_BUILTIN_TOOLS` (`src/core/builtin-tools.js`) must contain `Task`. Worklab always sends a named `allowedTools` array, agent-runtime turns it into `--tools <csv>`, and Claude Code hard-limits itself to that list — so omitting it silently disables every on-disk agent profile. Claude Code accepts `Task` or `Agent` in `--tools` and resolves both to the same tool, but every *event* reports `name: "Agent"`, which is what the UI matches on. Unrecognized names are ignored rather than rejected, so entries a backend lacks are free. `TodoWrite` is deliberately excluded — `core/run-todos.js` owns run todos.
- Per backend, `src/core/ai.js` sends only what lets that runtime find its own subagents: `settingSources: ["user","project","local"]` for the Claude SDK (which otherwise loads no filesystem settings, so it would never see `.claude/agents`; the Claude Code CLI already reads them because Worklab passes no `--setting-sources`), and the in-process `subagents` built-in with inline authoring for Pi (no on-disk profile concept, `supports_native_subagents: false`). Codex uses its own native surface. `settingSources` needs agent-runtime **> 0.17.1**; older builds ignore it and keep full isolation.
- `subagent_activity` is the runtime's nested-activity contract: `{type, subagent: {id, name, callIndex, toolUseId, label?}, phase, ...}` where `phase` is `agent_started | started | completed | agent_completed | message`. Claude Code runs a subagent as a *background task* — the parent's `Agent` tool returns "launched" immediately and the child streams nothing into the parent's stdout — so agent-runtime correlates `system/task_started` + `task_notification` and replays the child transcript at `task_notification.output_file`. The worker coalescer forwards these unchanged (its unknown-type path flushes pending parent text first, so a child never merges into the parent's prose); `AgentEventTimeline.jsx` folds them into one collapsible group per delegation, attached by `subagent.toolUseId`. The raw `task_started`/`task_updated`/`task_notification`/`background_tasks_changed` system events are hidden in the UI because `subagent_activity` supersedes them.
