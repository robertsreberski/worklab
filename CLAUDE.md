...
- Non-empty runtime skill sets must include the directory containing each `<name>/SKILL.md` as `skillsRoot`. `src/core/ai.js` derives the common root from Worklab's disclosed skill assets and also retains `skillDirs` for filesystem access boundaries; do not rely on the runtime's legacy `dataDir` fallback.
- Agent-runtime 0.17 added optional same-model route attempts and request-scoped `toolEnvironment`. Worklab does not opt into those: fallback chains retain one attempt per entry and the worker owns its environment. `src/__tests__/core/ai-runtime-options.test.js` guards that boundary.
- **Subagents are not the team roster.** A team roster exists so a planner can create durable Worklab child tasks through `decision: "delegate"`. Native subagents are ephemeral helpers owned by the active runtime: Claude discovers user/project/local agent profiles, Codex uses its collaboration tools, and Pi exposes bounded inline helpers. These were one mechanism until v49; do not reintroduce roster-derived runtime profiles or reuse the retired `subagent_mode` field. Its database column is only a one-release downgrade tombstone and must stay absent from API/MCP reads and writes.
- `WORKLAB_BUILTIN_TOOLS` (`src/core/builtin-tools.js`) must contain the exact `Agent` name as well as the Claude-compatible `Task` alias, `TaskOutput`, `TaskStop`, and `Skill`. Worklab always sends a named `allowedTools` array, so omitting a native entry silently removes that CLI capability. `TaskOutput` and `TaskStop` inspect or cancel runtime-local background helpers; they do not create Worklab tasks. Keep the other supported native helpers too, but exclude `TodoWrite` and Claude's task-state tools because `core/run-todos.js` and Worklab's durable task model own that state.
- `src/core/ai.js` sends one provider-option bag across every fallback attempt: `settingSources: ["user","project","local"]`, bounded Pi `subagents`, and `codexLoadProjectDocs: true`. Each adapter ignores fields it does not own. The Claude setting sources opt into repository/user profiles and can also load their hooks/plugins; managed policy still applies even when the list is empty, and Worklab's Skills allowlist does not sandbox provider-native settings. Pi child built-ins are the parent effective allowlist intersected with `Read`, `Glob`, `Grep`, `WebFetch`, and `WebSearch`; the child may also read the same Worklab-filtered skills already disclosed to its parent. Codex project docs are enabled so native child threads see repository instructions. This is an installed-package contract pinned and tested against exactly `@mono-agent/agent-runtime@0.18.2`.
- Fallback routes share the logical request but not a tool-policy representation. `createRouterRuntime.resolveAttempt().policyOptions` projects `allowedTools`, `disallowedTools`, and `permissionMode` for every actual route; its `options` also restores attempt-scoped custom Pi provider/model metadata that the router intentionally strips before resolution. Chain entries carry the correct default execution mode (`cli` for Codex/OpenCode, `sdk` for Claude/Pi). Never pass a primary Codex `['*']` into Claude or a Claude named list into Codex.
- `subagent_activity` is the runtime's nested-activity contract: `{type, subagent: {id, nativeId?, name, callIndex, label?, agentPath?, costUsd?}, phase, ...}` where `id` is the initiating parent Agent/spawn tool-use id and `phase` is `agent_started | started | completed | message | agent_completed`. `nativeId` is diagnostic provider identity only. Claude child frames are normalized live through `parent_tool_use_id`; Codex child-thread notifications are scoped away from the root result and projected into the same group. Child text never becomes parent output. The worker coalescer forwards these events unchanged; `AgentEventTimeline.jsx` attaches only to recognized Agent/spawn calls, renders deeper Codex paths flat inside that group, and lets child failure override an earlier successful launch result. Live tails count a delegation as one outer item, preserve lifecycle bookends, and retain at most 200 nested rows with an omitted count.
...
- The runtime owns the Pi dependency. Production code must not import `@earendil-works/pi-ai` — use the façade on `@mono-agent/agent-runtime/ai` (`listPiBuiltinModels`, `getPiBuiltinModel`, `reasoningLevelsForPiModel`, `resolvePiOAuthApiKey`, `loginPiOAuth`), which returns cloned snapshots and keeps the exact `0.83.0` pin inside the runtime. `eslint.config.js` enforces this; pi-ai stays a devDependency purely for the faux-provider fixtures in `src/__tests__/e2e/multi-sdk.test.js`. `loginPiOAuth` throws unless all four of `onAuth`, `onDeviceCode`, `onPrompt`, `onSelect` are supplied. Pi 0.83's `Provider is not configured` credential error must remain classified as `provider_auth` so fallback chains advance; the real two-route guard lives in `src/__tests__/ai/pi-sdk.test.js`.
