...
- `AGENTS.md` and `CONTRIBUTING.md` cite `docs/audits/task-agent-logic-audit.md` as a current reference, but that file is absent in this checkout (`AGENTS.md:20-22`, `CONTRIBUTING.md:3-10`; verified by filesystem check).
- `src/ui/README.md` says `npm run dev:ui` proxies to whatever `dev:api` is on and defaults to port `4280`, but `src/ui/vite.config.js` defaults the API proxy target to `WORKLAB_PORT || "7878"` and the Vite UI port to `5173` (`src/ui/README.md:6-8`, `src/ui/vite.config.js:14-29`).
- `docs/npm-release.md` lists publishable packages as `@worklab-ai/agent-runtime` and `@worklab-ai/worklab`, but the workflow now packs and verifies `packages/webhooks` / `@worklab-ai/webhooks` too (`docs/npm-release.md:10-15`, `.github/workflows/npm-release.yml:46-50`, `.github/workflows/npm-release.yml:71-77`, `packages/webhooks/package.json:1-46`).
- `docs/ai-runtime-bridge.md` tells new runtime authors to register a bridge in `src/ai/runtime/registry.js`; the live registry is in `packages/agent-runtime/src/ai/runtime/registry.js`, and `src/ai/runtime/registry.js` is absent in this checkout (`docs/ai-runtime-bridge.md:95-110`, `packages/agent-runtime/README.md:170-180`, `packages/agent-runtime/src/runtime.js:25-63`).
- `CLAUDE.md` refers to the runtime package as `@worklab/agent-runtime` in its boundary prose, while the manifest and source imports use `@worklab-ai/agent-runtime` (`CLAUDE.md:13-18`, `package.json:76`, `packages/agent-runtime/package.json:1-31`, `src/core/ai.js:1-10`).
