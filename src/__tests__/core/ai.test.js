...
describe("explicit model references", () => {
  ...
  it("advertises the current pi-ai OpenAI and Codex catalogues", () => {
    ...
    expect(getBuiltinModels().map((model) => model.value)).toContain("claude:claude-opus-5");
  });
  ...
  it("advertises per-model reasoning effort levels", () => {
    ...
    expect(getBuiltinModelByReference("claude:claude-fable-5").capabilities).toMatchObject({
      reasoning_levels: ["low", "medium", "high", "xhigh", "max"],
      context_window_tokens: 1_000_000,
      supports_1m_context: false,
    });
    expect(getBuiltinModelByReference("claude:claude-opus-5").capabilities).toMatchObject({
      reasoning_levels: ["low", "medium", "high", "xhigh", "max"],
      context_window_tokens: 1_000_000,
      supports_1m_context: false,
