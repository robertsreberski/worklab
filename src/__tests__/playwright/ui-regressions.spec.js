...
test("dropdown flips above its trigger when there is no room below", async ({ page }) => {
  ...
  // Push the trigger as close to the viewport bottom as possible so the
  // popover can't fit below it.
  await page.evaluate(() => {
    const trigger = document.querySelector(".skill-detail-body .form-field .select-trigger");
    if (!trigger) return;
    const rect = trigger.getBoundingClientRect();
    const target = window.innerHeight - rect.height - 16;
    const scrollContainer = trigger.closest(".scroll-container, .pane-detail-body, [data-scrollable]") || document.scrollingElement;
    if (scrollContainer && typeof scrollContainer.scrollBy === "function") {
      scrollContainer.scrollBy(0, rect.top - target);
    } else {
  ...
  const positions = await page.evaluate(() => {
    ...
    return {
      ...
      menuBottom: menuEl?.getBoundingClientRect().bottom || 0,
    };
  });
  expect(positions.menuTop).toBeLessThan(positions.triggerTop);
...
test("assistant composer keeps a compact bottom spacing regardless of keyboard state", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  // var(--sp-2) is 8px in the design system; the composer should never grow past it.
