...
// is nothing usable to resume from.
export function buildTranscriptTailSnapshot(events, {
  ...
  const tailTurns = turns.slice(-Math.max(1, Number(maxTurns) || DEFAULT_MAX_TURNS));
  const snapshot = {
  ...
  // Fall back to fewer turns if the JSON exceeds maxChars.
  let trimmedTurns = tailTurns.slice();
...
export function renderResumeSnapshot(snapshot) {
  ...
  lines.push("Below is a summary of the most recent work so you can continue from where it left off rather than starting over. Tool results are abbreviated; re-read files as needed if you require the full content.");
  lines.push("");
  snapshot.turns.forEach((turn, index) => {
    const label = `Turn ${snapshot.turn_count - snapshot.turns.length + index + 1}`;
    lines.push(`### ${label}`);
    if (turn.thinking) {
