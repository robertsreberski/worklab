...
import {
  searchMentionAgents,
  searchMentionGoals,
  searchMentionProjects,
  searchMentionRuns,
  searchMentionTasks,
  searchMentionTeams,
} from "../../core/db/queries/mentions.js";
...
function searchType(type, { db, dataDir, q, limit }) {
  ...
  if (!db) return [];
  if (type === "goal") {
    return searchMentionGoals(db, q, limit);
  }
  if (type === "run") {
    return searchMentionRuns(db, q, limit);
  }
  if (type === "agent") {
    return searchMentionAgents(db, q, limit);
  }
  if (type === "task") {
    return searchMentionTasks(db, q, limit);
  }
  if (type === "project") {
    return searchMentionProjects(db, q, limit);
  }
  if (type === "team") {
    return searchMentionTeams(db, q, limit);
  }
  return [];
}
