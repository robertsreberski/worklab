...
export function leadCycleFromRow(row) {
  ...
  return {
    ...
    task_creation_skips: jsonArray(row.task_creation_skips_json),
    goal_refinement: jsonObject(row.goal_refinement_json),
    goal_refinement_applied: jsonObject(row.goal_refinement_applied_json),
...
export function recordLeadCycleCompleted(db, {
  ...
  updateLeadCycleByRunId(db, runId, [
    ...
    "task_deletions_json = ?",
    "task_creation_skips_json = ?",
    "goal_refinement_json = ?",
    "goal_refinement_applied_json = ?",
    "advisory_notes_json = ?",
    ...
    JSON.stringify(Array.isArray(taskCreationSkips) ? taskCreationSkips : []),
    JSON.stringify(result?.goal_refinement || {}),
    JSON.stringify(goalRefinementApplied || result?.goal_refinement_applied || {}),
