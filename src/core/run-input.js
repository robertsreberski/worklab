...
import { parseStoredAllowlist, resolveAllowlist, resolveAllowlistMap, storedAllowlistMode } from "@mono-agent/agent-runtime/agent/allowlists.js";
...
export function buildNextTaskRunPreview({ db, config, taskId, now = Date.now(), worklabToolSurfaceMarkdown = "" }) {
  ...
  const metadata = {
    ...
    context_window: agent.context_window || "default",
