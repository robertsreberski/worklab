...
export function iconSvgMarkup(name, options = {}) {
  ...
  const safeStrokeWidth = Number.isFinite(Number(strokeWidth)) ? Number(strokeWidth) : 1.7;
  const classAttr = className ? ` class="${String(className).replace(/"/g, "&quot;")}"` : "";
  return `<svg${classAttr} width="${safeSize}" height="${safeSize}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${safeStrokeWidth}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${svg}</svg>`;
}
