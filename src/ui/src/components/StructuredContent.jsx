...
export function StructuredContent({
  ...
  if (segments.length === 1 && segments[0].type === "markdown") {
    return <MarkdownContent content={segments[0].text} className={className} maxHeight={maxHeight} expandable={expandable} />;
  }
  return (
    <div class={`structured-content ${className}`.trim()}>
      {segments.map((segment, index) => (
        segment.type === "structured"
          ...
          : <MarkdownContent key={index} content={segment.text} className="structured-markdown doc-content" maxHeight={maxHeight} expandable={expandable} />
      ))}
    </div>
  );
}
