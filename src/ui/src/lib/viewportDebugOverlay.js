...
function snapshot() {
  ...
  return {
    ...
    assistantFallbackLift: dockStyles?.getPropertyValue("--assistant-keyboard-fallback-lift").trim(),
    assistantLiftMode: dock?.dataset?.assistantKeyboardMode || "none",
    assistantKeyboardEstimate: dock?.dataset?.assistantKeyboardEstimate,
    assistantKeyboardTargetBottom: dock?.dataset?.assistantKeyboardTargetBottom,
...
function renderRows(snap) {
  const rows = [
    ...
    ["keybd ", `root=${fmt(snap.rootKeyboard)}  lift=${fmt(snap.assistantLift)}  fallback=${fmt(snap.assistantFallbackLift)}  est=${fmt(snap.assistantKeyboardEstimate)}`],
    ["mode  ", `assistant=${fmt(snap.assistantLiftMode)}  focused=${snap.assistantFocused}  target=${fmt(snap.assistantKeyboardTargetBottom)}  visible.bot=${fmt(snap.visibleBottom)}  overlap=${fmt(snap.composerOverlap)}`],
