...
async function notificationStatus(fetchImpl) {
  const response = await fetchImpl("/api/notifications/status");
  if (!response.ok) throw new Error(`notification status failed: ${response.status}`);
  return response.json();
}

async function deleteSubscription(fetchImpl, endpoint) {
  if (!endpoint) return;
  await fetchImpl("/api/notifications/subscriptions", {
    method: "DELETE",
...
async function subscribeWithServerKey(self, fetchImpl) {
  const status = await notificationStatus(fetchImpl);
...
export function installNotificationHandlers(self, { fetch: fetchImpl = self.fetch?.bind(self) } = {}) {
  ...
  self.addEventListener("pushsubscriptionchange", (event) => {
    event.waitUntil((async () => {
      ...
      const next = event.newSubscription || await subscribeWithServerKey(self, fetchImpl);
      if (oldEndpoint && oldEndpoint !== next?.endpoint) await deleteSubscription(fetchImpl, oldEndpoint);
      await saveSubscription(fetchImpl, next);
    })());
  });
